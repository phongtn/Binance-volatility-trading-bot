...
def load_correct_creds(creds, TEST=True):
    try:
        binance_creds = creds['binance']
        if TEST:
            return binance_creds['test']['access_key'], binance_creds['test']['secret_key']
        else:
            return binance_creds['prod']['access_key'], binance_creds['prod']['secret_key']

    except TypeError as te:
        message = 'Your credentials are formatted incorrectly\n'
        message += f'TypeError:Exception:\n\t{str(te)}'
        exit(message)
    except Exception as e:
        message = 'oopses, looks like you did something real bad. Fallback Exception caught...\n'
        message += f'Exception:\n\t{str(e)}'
        exit(message)
